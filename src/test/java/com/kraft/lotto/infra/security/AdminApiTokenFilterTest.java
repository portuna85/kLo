...
class AdminApiTokenFilterTest {
    ...
    @Test
    @DisplayName("Uses constant-time token comparison helper")
    void usesConstantTimeComparisonHelper() {
        byte[] a = "secret-token".getBytes(StandardCharsets.UTF_8);
        byte[] b = "secret-token".getBytes(StandardCharsets.UTF_8);
        byte[] c = "secret-token-x".getBytes(StandardCharsets.UTF_8);

        assertThat(AdminApiTokenFilter.constantTimeEquals(a, b)).isTrue();
        assertThat(AdminApiTokenFilter.constantTimeEquals(a, c)).isFalse();
    }

    @Test
    @DisplayName("Allows hashed token entry with token id")
    void allowsHashedTokenEntry() throws Exception {
        String token = "hash-token-value";
        String hash = "88c8daeadadab7fb3a0d46b6c71eab84b5f77bb88cd9a471cef45e7c26643c17";
        AdminApiTokenFilter hashedFilter = new AdminApiTokenFilter(
                new KraftAdminProperties("", "", "ops-token:" + hash, "X-Kraft-Admin-Token"),
                new KraftRecommendRateLimitProperties(),
                new ObjectMapper(),
                new SimpleMeterRegistry()
        );
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/admin/lotto/draws/collect-next");
        request.addHeader("X-Kraft-Admin-Token", token);
        MockHttpServletResponse response = new MockHttpServletResponse();
        FilterChain chain = mock(FilterChain.class);

        hashedFilter.doFilter(request, response, chain);
